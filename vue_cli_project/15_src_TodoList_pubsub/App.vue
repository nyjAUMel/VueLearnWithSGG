...
export default {
  ...
  mounted() {
    // 这里写了回调函数调用了组件的函数
    this.pubsubCheckTodoId = pubsub.subscribe("checkTodo", (msgName, data) => {
      this.checkTodo(data);
    });
    // 这里直接调用了组件的函数，并且改了组件的函数的参数。因为消息定于第一个参数是消息名
    this.pubsubDeleteTodoId = pubsub.subscribe("deleteTodo", this.deleteTodo);
  },
  beforeDestroy() {
    pubsub.unsubscribe(this.pubsubCheckTodoId);
    pubsub.unsubscribe(this.pubsubDeleteTodoId);
  },
  methods: {
    addTodo(todoObj) {
  ...
  watch: {
    todos: {
      deep: true,
      // val 是整个数组的新引用，不是你新加的那一项，而是整个数组的新内容。
      handler(val) {
